...
\Algorithm
{Template}
{Ez win template: go to St. Petersburg, write template, solve problems, submit solutions, get OK, win NEERC}
{$\mathcal{O}(1)$}
{cpp}{source/tempate/template.cpp}

\Algorithm
{Stress test}
{When you feel bad, you run this code and go walk with green badge, and get OK}
{$\mathcal{O}(\infty)$}
{cpp}{source/tempate/stress.cpp}
...
{$\mathcal{O}(n logn)$}
{cpp}{source/geometry/half_plane.cpp}

\Algorithm
{Closest pair}
{Can be modificated to find triangle with min perimiter}
{$\mathcal{O}(n logn)$}
{cpp}{source/geometry/pair.cpp}
